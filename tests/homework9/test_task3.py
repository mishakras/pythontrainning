...
def test_without_tokens_one_file(tmpdir):
    ...
    assert universal_file_counter(tmpdir, 'txt') == 4
...
def test_without_tokens_some_file(tmpdir):
    ...
    assert universal_file_counter(tmpdir, 'txt') == 12


def test_with_tokens_one_file(tmpdir):
    tmpdir.join('test31.txt').write('1 2 3 4')
    assert universal_file_counter(tmpdir, 'txt', str.split) == 4
...
def test_with_tokens_some_file(tmpdir):
    ...
    assert universal_file_counter(tmpdir, 'txt', str.split) == 11
